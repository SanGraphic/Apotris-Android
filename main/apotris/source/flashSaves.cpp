...
#define FLASH_CODE __attribute__((noinline, section(".ewram.flash")))
#define BYTECMP_CODE __attribute((section(".ewram.bytecmp")))
...
BYTECMP_CODE static int bytecmp(vu8* buf1, vu8* buf2, size_t count) {
...
FLASH_CODE u32 get_flash_type() {

    // test if able to write to SRAM/FRAM
    loadSavefile();

    if (savefile->newGame >= 0x4b && savefile->newGame <= SAVE_TAG) {
        if (savefile->newGame < 0x51) {
            return 0;
        } else if (savefile->settings.bootlegOverrideType) {
            return savefile->settings.bootlegOverrideType;
        }
        escape = true;
        return 0;
    }
...
FLASH_CODE void flash_write(u8 flash_type, u32 sa) {
    if (flash_type == 0)
        return;
    u16 ie = REG_IE;
    // stop_dma_interrupts();
    ...
    if (flash_type == 1) {
        ...
        for (int i = 0; i < AGB_SRAM_SIZE; i += 2) {
            ...
            _FLASH_WRITE(sa + i, (*(u8*)(AGB_SRAM + i + 1)) << 8 |
                                     (*(u8*)(AGB_SRAM + i)));
            while (1) {
                __asm("nop");
                if (*(((u16*)AGB_ROM) + (sa / 2)) == 0x80) {
        ...
        while (c < AGB_SRAM_SIZE) {
            ...
            for (int i = 0; i < 1024; i += 2) {
                _FLASH_WRITE(sa + c + i, (*(u8*)(AGB_SRAM + c + i + 1)) << 8 |
                                             (*(u8*)(AGB_SRAM + c + i)));
            }
...
void detect_rom_backed_flash() {
    ...
    bootleg_type = get_flash_type();
    savefile->settings.bootlegOverrideType = bootleg_type;
    if (bootleg_type > 0) {
        ...
        __agbabi_memcpy1((void*)AGB_SRAM,
                         (const void*)((vu8*)AGB_ROM + flash_sram_area),
                         AGB_SRAM_SIZE);
        ...
        if ((*(vu32*)(AGB_ROM + 0x400000 - 0x40000) == STATEID) ||
            (*(vu32*)(AGB_ROM + 0x400000 - 0x40000) == STATEID2)) {
            flash_sram_area = 0x400000 - 0x40000;
        } else if ((*(vu32*)(AGB_ROM + 0x800000 - 0x40000) == STATEID) ||
                   (*(vu32*)(AGB_ROM + 0x800000 - 0x40000) == STATEID2)) {
            flash_sram_area = 0x800000 - 0x40000;
        } else if ((*(vu32*)(AGB_ROM + 0x1000000 - 0x40000) == STATEID) ||
                   (*(vu32*)(AGB_ROM + 0x1000000 - 0x40000) == STATEID2)) {
            flash_sram_area = 0x1000000 - 0x40000;
        } else if ((*(vu32*)(AGB_ROM + 0x2000000 - 0x40000) == STATEID) ||
                   (*(vu32*)(AGB_ROM + 0x2000000 - 0x40000) == STATEID2)) {
...
void save_sram_flash() {
    ...
    flash_write(bootleg_type, flash_sram_area);
    ...
    aprint("   Saved!", 21, 19); // spaces necessary to "erase" the previous
                                 // text
}
#endif
#endif
